...
using TakeFoodAPI.Model.Content;
using TakeFoodAPI.Model.Entities;
using TakeFoodAPI.Utilities;
    ...
    : IMongoRepository<T> where T : ModelMongoDB
{
    ...
    Task<PagingData<T>> IMongoRepository<T>.GetPagingAsync(FilterDefinition<T> filter, int pageNumber, int pageSize, string sortColumn, string sortType, bool includeIsDeleted)
    {
        throw new NotImplementedException();
    }

    Task<PagingData<T>> IMongoRepository<T>.GetPagingAsync(FilterDefinition<T> filter, int pageNumber, int pageSize, SortDefinition<T> sort, bool includeIsDeleted)
    {
        throw new NotImplementedException();
    }
}
